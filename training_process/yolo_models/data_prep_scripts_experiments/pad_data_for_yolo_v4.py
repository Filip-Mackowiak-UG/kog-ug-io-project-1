...
directory = "./huge_data/cards_kaggle_yolo_padded_v4/valid"
...
for filename in os.listdir(directory):
    if filename.endswith(".jpg"):
        ...
        # Add padding
        pad = random.randint(5, img.shape[0])  # Use the same padding for both dimensions to maintain aspect ratio

        # Create a random-colored background image
        bg = np.random.randint(0, 256, (img.shape[0] + 2 * pad, img.shape[1] + 2 * pad, 3), dtype=np.uint8)

        # Place the original image in the center of the background image
        bg[pad:-pad, pad:-pad] = img
        ...
        with open(txt_path, "r") as f:
            bboxes = f.readlines()
        bboxes = adjust_bboxes(bboxes, pad, pad, img.shape[0])  # Use original image size for adjusting bboxes
